...
CREATE OR REFRESH STREAMING TABLE ubereats.bronze.tb_events(
  CONSTRAINT no_rescued_data
  EXPECT (_rescued_data IS NULL)
)
COMMENT "Raw Kafka events from JSON files. Bronze layer - preserves all data as-is with metadata tracking."
TBLPROPERTIES (
  "quality" = "bronze",
  "source_system" = "kafka",
  "layer" = "raw_ingestion"
...
AS SELECT *,
 ...
 -- Auto Loader metadata for lineage tracking
  ...
  _metadata.file_modification_time AS ingestion_time
FROM STREAM read_files(
  '/Volumes/ubereats/raw/datalake-ubereats/json-files/json-files/kafka_events_*',
  format => 'json'
);

CREATE OR REFRESH STREAMING TABLE ubereats.bronze.tb_gps(
  CONSTRAINT no_rescued_data
  EXPECT (_rescued_data IS NULL)
)
COMMENT "Raw Kafka gps from JSON files. Bronze layer - preserves all data as-is with metadata tracking."
TBLPROPERTIES (
  "quality" = "bronze",
  "source_system" = "kafka",
  "layer" = "raw_ingestion"
...
AS SELECT *,
 ...
 -- Auto Loader metadata for lineage tracking
  ...
  _metadata.file_modification_time AS ingestion_time
FROM STREAM read_files(
  '/Volumes/ubereats/raw/datalake-ubereats/json-files/json-files/kafka_gps_*',
  format => 'json'
);
...
CREATE OR REFRESH STREAMING TABLE ubereats.bronze.tb_receipts(
  CONSTRAINT no_rescued_data
  EXPECT (_rescued_data IS NULL)
)
COMMENT "Raw Kafka receipts from JSON files. Bronze layer - preserves all data as-is with metadata tracking."
TBLPROPERTIES (
  "quality" = "bronze",
  "source_system" = "kafka",
  "layer" = "raw_ingestion"
...
AS SELECT *,
 ...
 -- Auto Loader metadata for lineage tracking
  ...
  _metadata.file_modification_time AS ingestion_time
FROM STREAM read_files(
  '/Volumes/ubereats/raw/datalake-ubereats/json-files/json-files/kafka_receipts_*',
  format => 'json'
);